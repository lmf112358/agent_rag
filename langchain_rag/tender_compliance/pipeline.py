...
class TenderCompliancePipeline:
    ...
    def run(
        ...
        logger.info(f"")
        logger.info(f"{'='*60}")
        logger.info(f"   标书审核Pipeline启动")
        logger.info(f"{'='*60}")
        logger.info(f"   项目名称: {project_name or '未命名项目'}")
        logger.info(f"   项目类型: {project_type}")
        logger.info(f"   投标公司: {company_name or '未命名公司'}")
        logger.info(f"   招标书ID: {tender_id}")
        logger.info(f"   投标书ID: {bid_id}")
        logger.info(f"   招标书: {Path(tender_pdf).name}")
        logger.info(f"   投标书: {Path(bid_pdf).name}")
        logger.info(f"   KB Verify: {'启用' if self.enable_kb_verify else '禁用'}")
        logger.info(f"{'='*60}")
        logger.info(f"")
        ...
        stage_times["Stage 1"] = time.time() - stage1_start
        logger.info(f"")

        # ==================== Stage 2: 条款对齐层 ====================
        logger.info(f">>> [Stage 2/5] 条款对齐层")
        stage2_start = time.time()
        checklist, bid_response = self.stage2_aligner.align(
            tender_doc=tender_doc,
            bid_doc=bid_doc
        )
        stage_times["Stage 2"] = time.time() - stage2_start
        logger.info(f"")

        # ==================== Stage 3: 核对引擎层 ====================
        logger.info(f">>> [Stage 3/5] 核对引擎层")
        stage3_start = time.time()
        compliance_result = self.stage3_compliance.check(
            checklist=checklist,
            bid_response=bid_response
        )
        stage_times["Stage 3"] = time.time() - stage3_start
        logger.info(f"")

        # ==================== Stage 4: 评分汇总层 ====================
        logger.info(f">>> [Stage 4/5] 评分汇总层")
        stage4_start = time.time()
        scoring_card = self.stage4_scoring.score(
            checklist=checklist,
            compliance_result=compliance_result
        )
        stage_times["Stage 4"] = time.time() - stage4_start
        logger.info(f"")

        # ==================== Stage 5: 人工复核层 ====================
        logger.info(f">>> [Stage 5/5] 人工复核层")
        ...
        stage_times["Stage 5"] = time.time() - stage5_start
        logger.info(f"")
