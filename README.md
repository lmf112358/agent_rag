...
6. [报价审核Agent](#报价审核agent)
7. [MinerU 文档解析集成](#mineru-文档解析集成)
7. [配置详解](#配置详解)
8. [API 文档](#api-文档)
9. [知识库管理](#知识库管理)
10. [测试与验证](#测试与验证)
...
| **报价审核Agent** | 5阶段Pipeline实现工程量清单自动化审核（GB 50500） | ✅ |
...
报价审核Agent是一个5阶段Pipeline的自动化工程量清单审核系统，基于《建设工程工程量清单计价规范》GB 50500，将原本需要人工核算的报价审核工作缩短到分钟级。

**解决的核心痛点：**
- **报价精准度风险**：200+ 分项报价，LLM 幻觉导致漏算
- **算术错误遗漏**：工程量×单价=合价的人工核算耗时且易错
- **造价异常难发现**：费用占比、材料/人工费比例异常难以识别

**核心功能：**
| 功能模块 | 说明 | 状态 |
|---------|------|------|
| **Layer 1: 数据清洗** | Excel 解析、空值/重复检测、备注解析 | ✅ |
| **Layer 2: GB 50500 合规** | 单位一致性、特征完整性、甲供校验 | ✅ |
| **Layer 3: 算术校验** | 横向量价校验、纵向汇总校验、费用层级校验 | ✅ |
| **Layer 4: 造价指标分析** | 单方造价、占比分析、历史对标 | ✅ |
| **Layer 5: 报告生成** | 问题分级、状态判定、Markdown 报告 | ✅ |
...
# 上传 Excel 文件进行审核
POST /api/quote/audit
Content-Type: multipart/form-data

file: 工程量清单.xlsx
project_name: 项目名称
total_rt: 1650      # 可选：总制冷量
building_area: 5000 # 可选：建筑面积
```
...
report = pipeline.run(
    excel_path="data/工程量清单.xlsx",
    project_name="珠海某PCB厂高效机房项目",
    total_rt=1650,
    building_area=5000
)

print(f"审核状态: {report.overall_status}")
...
| 参数 | 作用 | 说明 |
|------|------|------|
| **TotalRT** | 计算元/RT | 总制冷量，用于单位造价对标 |
| **BuildingArea** | 计算元/㎡ | 建筑面积，辅助判断造价水平 |

### 项目文件结构

```
langchain_rag/quote_audit/
├── __init__.py              # 模块初始化
├── config.py                # 费率配置、列映射
├── models.py                # Pydantic数据模型
├── pipeline.py              # 主Pipeline类
├── excel_parser.py          # Excel解析器
├── stage1_cleansing.py     # Layer 1: 数据清洗
├── stage2_gb50500.py        # Layer 2: GB 50500合规
├── stage3_arithmetic.py     # Layer 3: 算术校验
├── stage4_cost_index.py     # Layer 4: 造价指标
└── stage5_report.py         # Layer 5: 报告生成

backend/services/
└── quote_service.py         # 报价审核API服务
```

### 更多帮助

详见：`docs/02-核心技术路径/报价审核技术路径.md`
...
| `/api/quote/audit` | POST | 报价审核（Excel上传） | 否 |
...
```json
{
  "success": true,
  "data": {
    "report_id": "8858b63c-7217-48c5-90a5-1b5c8aadf473",
    "project_name": "珠海某PCB厂高效机房项目",
...
│   ├── quote_audit/            # 报价审核 5 阶段 Pipeline
│   │   ├── __init__.py
│   │   ├── models.py          # Pydantic 数据模型
│   │   ├── config.py          # 费率配置、列映射
│   │   ├── pipeline.py        # 主 Pipeline
...
│   │   ├── stage1_cleansing.py # 数据清洗
│   │   ├── stage2_gb50500.py  # GB 50500 合规
│   │   ├── stage3_arithmetic.py # 算术校验
│   │   ├── stage4_cost_index.py # 造价指标
│   │   └── stage5_report.py   # 报告生成
│   ├── tests/                  # 单元测试
│   │   ├── conftest.py
│   │   ├── test_qwen.py
│   │   ├── test_vectorstore.py
